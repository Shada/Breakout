...
{
		float angle		= 1.0f - dot( normal, eyeDir );
		float fresnel	= angle * angle;
		fresnel			= fresnel * fresnel;
		fresnel			= fresnel * angle;
		return fresnel * ( 1.0f - .5f ) + .5f - refractionStrength;
}
...
void main()
{
	...
	vec4 worldPos = viewInv * vec4( (viewPos / viewPos.w).xyz,1);
	...
	/*mat4 mTextureProj	= projection * view * mReflection;
	vec3 waterPos			= surfacePos.xyz - level + waterLevel;
	vec4 texCoordProj		= mTextureProj * vec4(waterPos,1.0f);
	...
	vec3 waterColor = vec3(0, 0.4, 0.9);//saturate(length(vec3(1.0f,1.0f,1.0f)) / 3.0f);
	refraction = mix(mix(refraction, waterSurfaceColor.xyz * waterColor, 1-depthN / transparency),
						waterDepthColor.xyz * waterColor, depth2 / extinction.xyz);
	...
	{
		foam += (texture2D(foamMap, texCoord) + texture2D(foamMap, texCoord2)) * 0.05f * (level - (waterLevel + foamOptions.z)) / (maxAmplitude - foamOptions.z);
	}
	...
	color = mix(originalColor, mix(vec4(refraction,1.0f), vec4(reflection,1.0f),  0.7*fresnel) + vec4(specular,1) + foam, depth2 * shoreTransition);
}