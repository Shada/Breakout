...
void main()
{
	...
	{
		color = texture2D(texScene, vec2( UV.x + 2*sin(timer * 0.002f + 3.0f * abs(UV.y)) * (refractionScale * min(UV.y, 1.0f)), UV.y ) ) + vec4(0,0,0.3f,1);
		return;
	}
    ...
    if(worldPos.y > waterLevel - waterfix)// + maxAmplitude)//if the pixel is above the water surface, then discard.
	{
	if(waterType == 1) // makes glow on brick when close to lava
    {
        if(worldPos.y < (waterLevel + 10 - waterfix) && depth < 1)// + maxAmplitude)//if the pixel is above the water surface, then discard.
        {
            float oldrange = 10 - 0;
            float newrange = 1 - 0;
            float glowfactor = waterLevel + (10 - waterfix) - worldPos.y;
            glowfactor = (((glowfactor - 0) * newrange ) /oldrange) + 0;
            color = saturate(originalColor + vec4(1, 0.3f, 0.15, 1) * glowfactor);
            return;
        }
    }
        color = originalColor;
	...
	float spec = pow(max(dot(normalize(-s + 2 *dot(s, tnorm ) * tnorm ), normalize( cameraPosWV - vec3( viewPosRefr.xyz/viewPosRefr.w))), 0.0), 60);
	...
	vec4 lavaColor = texture2D(texLava, texCoord);
	float glowfactor = texture2D(lavaGlowMap, texCoord).r;
	float oldrange = 1 - (-1);
	float newrange = 1 - 0;
	float pulse = sin(timer * 0.0004f);
	pulse = (((pulse - (-1)) * newrange) / oldrange) + 0.5f;

	vec4 outcolor = saturate(mix(mix(originalColor * glowfactor, mix(vec4(refraction, 1), vec4(reflection, 1) * glowfactor, 0.7 * fresnel) + foam, saturate(depth2 * shoreTransition)), lavaColor, 0.6f));
	color = saturate(outcolor + vec4(1, 0.5f, 0.15f, 1) * pulse * glowfactor);
	//color = vec4(0,0,0,0);
}
